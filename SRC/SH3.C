 ...
 *
 *    $Header: /usr/users/istewart/src/shell/sh2.2/RCS/sh3.c,v 2.12 1993/11/09 10:39:49 istewart Exp $
 *
 *    $Log: sh3.c,v $
...
static int F_LOCAL	StartTheSession (STARTDATA *, char *, char **, char **);
static int F_LOCAL	OS2_DosExecProgram (int, char *, char **, char **,
					    unsigned int);
#endif
...
static OSCALL_RET	OS2_DosExecPgmReturnCode;
...
/* Check that OnlyDigits is a valid number in the select range */
			...
			(OnlyDigits < Count))
			SetVariableFromString (t->str, AList[OnlyDigits]);
...
/* Set up links */
	...
	case TCASE:			/* CASE function		*/
	{
		...
		for (AList = ts->vars; *AList != NOWORD; AList++)
		{
			...
			GeneralPatternMatch (cp, EndIB, FALSE, (char **)NULL,
					     GM_ALL))
			goto Found;
		}
...
/* Open the file in the appropriate mode */
	...
	case IOWRITE:				/* >			*/
	    ...
	    u = S_open (FALSE, cp, O_CMASK | O_BINARY);
	    break;
...
/* Get the driver interface */
    ...
    or.x.REG_AX = 0x4310;
    SystemSegInterrupt (0x2f, &or, &or, &sr);
...
/* Support for version 3 of XMS driver */
	...
	return FL_TEST (FLAG_NO_WARNING)
		? XMS_error ("Warning: %s Version < 2", 0) : FALSE;

    else if (SW_XMS_Available () < (SW_Blocks * 16))
	return FL_TEST (FLAG_NO_WARNING) ? XMS_error (MS_Space, 0) : FALSE;
...
/* If we find it - create a temporary file and write the stuff */
	...
	if ((found) &&
	    ((fd = OpenFile (Extend_file = GenerateTemporaryFileName (),
			     OpenWriteMode)) != (FILE *)NULL))
	{
...
/* In current directory ? */
    ...
    if ((p = strrchr (path, CHAR_UNIX_DIRECTORY)) == (char *)NULL)
    {
	 p = path;
...
/* In root directory */
	...
	*npath = *cpath;
    ...
    else if (((p - path) == 2) && (*(path + 1) == CHAR_DRIVE))
    {
	++p;
	strcpy (npath, RootDirectory);
	*npath = *path;
...
/* Change to the directory containing the executable */
	...
	drive = (*(path + 1) == CHAR_DRIVE) ? GetDriveNumber (*path)
#if (OS_VERSION != OS_DOS) && (OS_VERSION != OS_DOS_32) && !defined (__WATCOMC__)
					    : _getdrive ();
#else
					    : 0;
#endif
...
/* Restore our original directory */
    ...
    if (npath[strlen (npath) - 1] != CHAR_UNIX_DIRECTORY)
...
static int F_LOCAL StartTheProcess (char *path,
				    char **argv,
				    char **envp,
				    int ForkAction)
{
...
#  if (OS_VERSION == OS_DOS)
	retval = execve (path, ProcessSpaceInParameters (argv), envp);
#  else
...
/* In OS/2, need to set signal to default so child will process it */
    ...
    {
#if (OS_VERSION == OS_OS2_16)
	if ((usType & FAPPTYP_EXETYPE) == FAPPTYP_WINDOWAPI)
#else
	if (((usType & FAPPTYP_EXETYPE) == FAPPTYP_WINDOWAPI) ||
	    (usType & FAPPTYP_DOS))
#endif
...
#else
	    stdata.SessionType = (usType & FAPPTYP_DOS) ? SSF_TYPE_VDM
	    						: SSF_TYPE_PM;
#endif
 ...
 */
    ...
    DISABLE_HARD_ERRORS;
#if (OS_VERSION == OS_OS2_16) && !defined (OS2_16_BUG)
	...
	if (InteractiveFlag && (source->type == STTY))
	{
	    ...
	    if (usType)
		fprintf (stderr, "%s\n", GetOS2SystemErrorMessage (usType));
	}
    ...
    {
	strcpy (FailName, GetOS2SystemErrorMessage (usType));
...
static char	*Extensions [] = {
    null,		EXEExtension,		COMExtension,
    SHELLExtension,	KSHELLExtension,	BATExtension, };
...
/* Scan the path for an executable */
    ...
    sp = ((strchr (name, CHAR_UNIX_DIRECTORY) != (char *)NULL) ||
	  (*(name + 1) == CHAR_DRIVE)) ? null
...
/* Open the file */
    ...
    if ((LF.FP = OpenFile (sp, OpenReadMode)) == (FILE *)NULL)
    {
 ...
 * twice as much space, plus some nulls
 */

#if (OS_VERSION == OS_DOS) || (OS_VERSION == OS_DOS_32)
    if ((CmdLineLength = strlen (CommandLine)) >= CMD_LINE_MAX - 2)
#else
    if ((CmdLineLength = strlen (CommandLine))
    	>= (((CMD_LINE_MAX - 2) / 2) + CountNumberArguments (SavedArgs) + 3))
#endif
    {
	errno = E2BIG;
	RetVal = FALSE;
...
#if (OS_VERSION != OS_DOS) && (OS_VERSION != OS_DOS_32)
static int F_LOCAL OS2_DosExecProgram (int Mode, char *Program, char **argv,
...
/* Exec it - with hard-error processing turned off */
    ...
    OS2_DosExecPgmReturnCode = DosExecPgm (FailName, sizeof (FailName),
...
/* If the process started OK, wait for it */
	...
	if (((OS2_DosExecPgmReturnCode == NO_ERROR) ||
	     (OS2_DosExecPgmReturnCode == ERROR_INTERRUPT)) &&
	    (Mode == P_WAIT))
	{
 ...
 */
	    ...
	    while ((OS2_DosExecPgmReturnCode = DosCwait (DCWA_PROCESSTREE,
	    						 DCWW_WAIT,
							 &rescResults,
							 &pidProcess,
							 pidWait))
 ...
 */
    ...
    if (OS2_DosExecPgmReturnCode != 0)
    {
	switch (OS2_DosExecPgmReturnCode)
	{
#ifdef EAGAIN
...
static void F_LOCAL PrintLoadError (char *path)
{
    ...
    else if (OS2_DosExecPgmReturnCode)
	PrintWarningMessage ("%s: %s\n%s", path, ConvertErrorNumber(),
		     GetOS2SystemErrorMessage (OS2_DosExecPgmReturnCode));
 ...
 */
	...
	if (((GlobalFlags & FLAGS_MSDOS_FORMAT) ||
...
#if (OS_VERSION != OS_DOS) && (OS_VERSION != OS_DOS_32)
char	*GetOS2SystemErrorMessage (OSCALL_RET code)
{
...
static void F_LOCAL TrackAllCommands (char *path, char *arg)
{
	...
	(strchr (arg, CHAR_UNIX_DIRECTORY) == (char *)NULL) &&
	(arg[1] != CHAR_DRIVE))
	SaveAlias (arg, path, TRUE);
}
