 ...
 *
 *    $Header: /usr/users/istewart/src/shell/sh2.2/RCS/sh5.c,v 2.10 1993/11/09 10:39:49 istewart Exp $
 *
 *    $Log: sh5.c,v $
 ...
 */
    ...
    LexSeparator = (char)((LexicalControlFlags & TEST_EXPRESSION) ? C_IFS
								  : C_LEX1);
...
/* Create the tempoary file */
    ...
    if (((tf = S_open (FALSE, iop->io_name,
    		       O_CMASK | O_BINARY | O_NOINHERIT)) < 0)
	|| ((FP = ReOpenFile (tf, OpenWriteBinaryMode)) == (FILE *)NULL))
...
{
    Here_D	*h;
    Here_D	*hl = (Here_D *)NULL;

    for (h = ActiveListHead; h != (Here_D *)NULL; hl = h, h = h->h_next)
    {
 ...
 */
	...
	{
	    if ((FP = OpenFile (hname, OpenReadMode)) == (FILE *)NULL)
	    {
...
/* set up ScanNextToken input from here file */
    ...
    /* write expanded input to another temp file */
	    ...
	    if ((FP = OpenFile (tname, OpenAppendMode)) == (FILE *)NULL)
	    {
