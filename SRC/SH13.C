 ...
 *
 *
 *    $Header: /usr/users/istewart/src/shell/sh2.2/RCS/sh13.c,v 1.6 1993/12/02 09:29:04 istewart Exp $
 *
 *    $Log: sh13.c,v $
...
static void F_LOCAL	EMACS_Initialisation (void);
...
static int F_LOCAL	EMACS_SearchMatch (char *, int);
...
static void F_LOCAL	EMACS_MapOutKeystrokes (int);
...
static bool F_LOCAL	EMACS_CheckYankPop (void);
...
static void F_LOCAL	GEN_AjustOutputString (char *);
...
#define	EMACS_MEMORY_ALLOC	2	/* ALlocate memory		*/
#define	EMACS_NO_BIND		4	/* No binding			*/
...
static int		emacs_ArgumentCount;	/* general purpose arg	*/
 ...
 */
    ...
    {EMACS_GotoEnd,	"end-of-line",		0,	'E' & 0x01f,	0 },
    {EMACS_GotoStart,	"beginning-of-line",	0,	'A' & 0x01f,	0 },
    {EMACS_KillLine,	"kill-line",		0, 	'U' & 0x01f,	0 },

    {EMACS_NextCharacter,
			"forward-char",		0,	'F' & 0x01f,	0 },
    {EMACS_MoveForwardAWord,
			"forward-word",		1,	'F',		0 },

    {EMACS_PreviousCharacter,
			"backward-char",	0,	'B' & 0x01f,	0 },
    {EMACS_MoveBackAWord,
			"backward-word", 	1,	'B',		0 },

    {EMACS_DeleteCurrentCharacter,
			"delete-char-forward",	0,	0,		0 },
    {EMACS_DeleteNextWord,
			"delete-word-forward", 	1,	'D',		0 },

    {EMACS_DeleteCharacterBackwards,
			"delete-char-backward",	0,	CHAR_BACKSPACE,	0 },
 ...
 */
    ...
    {EMACS_GetPreviousCommand,
			"up-history",		0,	'P' & 0x01f,	0 },
    {EMACS_GetNextCommand,
			"down-history",		0,	'N' & 0x01f,	0 },
    {EMACS_SearchHistory,
			"search-history",	0,	'R' & 0x01f,	0 },
    ...
    {EMACS_ClearScreen,	"clear-screen",		0,	0,		0 },
    ...
    {EMACS_Transpose,	"transpose-chars",	0, 	'T' & 0x01f,	0 },
    ...
    {EMACS_CompleteFile,
			"complete",		1, 	CHAR_ESCAPE,	0 },
    {EMACS_SubstituteFiles,
			"complete-list",	1,	'*',		0 },
    {EMACS_ListFiles,	"list",			1,	'=',		0 },
...
#  endif
    ...
    {EMACS_SetArgValue,	"",			1,	'0',		0 },
    {EMACS_SetArgValue,	"",			1,	'1',		0 },
    {EMACS_SetArgValue,	"",			1,	'2',		0 },
    {EMACS_SetArgValue,	"",			1,	'3',		0 },
    {EMACS_SetArgValue,	"",			1,	'4',		0 },
    {EMACS_SetArgValue,	"",			1,	'5',		0 },
    {EMACS_SetArgValue,	"",			1,	'6',		0 },
    {EMACS_SetArgValue,	"",			1,	'7',		0 },
    {EMACS_SetArgValue,	"",			1,	'8',		0 },
    {EMACS_SetArgValue,	"",			1,	'9',		0 },
...
/* Check status */
    ...
    if ((fp = OpenFile ((Temp = GenerateTemporaryFileName ()),
			OpenWriteBinaryMode)) == (FILE *)NULL)
    {
...
static int F_LOCAL VI_ForwardWord (int argcnt)
{
    ...
    while (ncursor < VI_InputLength && argcnt--)
    {
	if (IS_AlphaNumeric (ConsoleLineBuffer[ncursor]))
	{
	    while (IS_AlphaNumeric (ConsoleLineBuffer[ncursor]) &&
	           (++ncursor < VI_InputLength))
		continue;
	}
...
static int F_LOCAL VI_BackwardWord (int argcnt)
{
	...
	if (ncursor > 0)
	{
	    ...
	    {
		while ((--ncursor >= 0) &&
		       IS_AlphaNumeric (ConsoleLineBuffer[ncursor]))
		    continue;
	    }
...
static void F_LOCAL VI_RedrawLine (void)
{
    ...
    memset (WindowBuffer[0], CHAR_SPACE, MaximumColumns - 3);
    memset (WindowBuffer[1], CHAR_SPACE, MaximumColumns - 3);
}
...
/* Initialise the window buffers */
	...
	WindowBuffer[c] = GetAllocatedSpace (MaximumColumns - 3);
	SetMemoryAreaNumber (WindowBuffer[c], 0);
	memset (WindowBuffer[c], CHAR_SPACE, MaximumColumns - 3);
    }
...
/*
 * GEN_AjustOutputString() may result in a call to GEN_AdjustRedraw ()
...
/* Copy including the null */
    ...
    GEN_AjustOutputString (emacs_CurrentPosition);
...
/* Search for a match */
	...
	if ((*cp == (char)c) && (emacs_ArgumentCount--))
...
{
    return EMACS_LoadFromHistory (CurrentHistoryEvent - 1);
}
...
static int F_LOCAL EMACS_SearchHistory (int c)
{
    ...
    *p = 0;
	...
	if (c == CHAR_ESCAPE)
	    ...
	    offset = EMACS_SearchMatch (pat, offset);

	else if (func == EMACS_DeleteCharacterBackwards)
	    continue;	/* todo */

	else if (func == EMACS_AutoInsert)
	{
...
static int F_LOCAL EMACS_SearchMatch (char *pat, int offset)
{
    int		event = CurrentHistoryEvent;
    char	*hp;
    int		i;
...
#  endif
	...
	GEN_BackspaceOver (emacs_CurrentPosition[-2]);
	GEN_OutputCharacterWithControl (emacs_CurrentPosition[-1]);
	GEN_OutputCharacterWithControl (emacs_CurrentPosition[-2]);
    ...
    {
	GEN_BackspaceOver (emacs_CurrentPosition[-1]);
	GEN_BackspaceOver (emacs_CurrentPosition[0]);
	GEN_OutputCharacterWithControl (emacs_CurrentPosition[-1]);
	tmp = emacs_CurrentPosition[-1];
	emacs_CurrentPosition[-1] = emacs_CurrentPosition[0];
	emacs_CurrentPosition[0] = tmp;
	GEN_BackspaceOver (emacs_CurrentPosition[0]);
    }
...
static void F_LOCAL EMACS_StackText (char *start, int nchars)
{
    ...
    if (emacs_Stack[emacs_StackPointer])
...
{
    if (!EMACS_CheckYankPop ())
	return EMACS_KEY_NORMAL;

    GEN_PutAString ("\nYankPop not implemented");
    GEN_Redraw (-1);
    return EMACS_KEY_NORMAL;
}

/*
 * Check previous command was a Yank/Pop
 */
...
static int F_LOCAL EMACS_PutText (int c)
{
    ...
    do
    {
	emacs_TopOfStack = (emacs_TopOfStack == 0) ? EMACS_KILL_SIZE - 1
						   : emacs_TopOfStack - 1;
    }  while (emacs_Stack[emacs_TopOfStack] == (char *)NULL);
...
static void F_LOCAL EMACS_MapOutKeystrokes (int c)
{
...
static void F_LOCAL EMACS_PrintMacros (int prefix, int key)
{
    ...
    {
	Quotes = TRUE;
	feputc (CHAR_SINGLE_QUOTE);
    }

    feputs (emacs_KeyDefinitions[prefix][key]->emacs_FunctionName);
...
static int F_LOCAL EMACS_FileCompletion (int type)
{
    char		buf [256 + 1];
    char		bug [256 + 1];
    char		*str = emacs_CurrentPosition;
    char		*cp = buf;
    ...
    *cp = 0;
    ...
    if ((lastp = strrchr (buf, CHAR_UNIX_DIRECTORY)) != (char *)NULL)
	*lastp = 0;

    dirnam = (lastp == (char *)NULL) ? "."
...
static void F_LOCAL GEN_Redraw (int limit)
{
    ...
    GEN_AjustOutputString (emacs_StartVisible);
...
static void F_LOCAL GEN_AjustOutputString (char *str)
{
    int		adj = AdjustDone;
