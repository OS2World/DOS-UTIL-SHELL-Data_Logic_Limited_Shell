 ...
 * Scanner.
 *
 *    $Header: /usr/users/istewart/src/shell/sh2.2/RCS/sh1.c,v 2.14 1993/12/01 11:58:34 istewart Exp $
 *
 *    $Log: sh1.c,v $
...
#  else
#    define RELEASE_MEMORY(x)	{ memset (x, 0, sizeof (s_region));	\
				  free ((x)); }
#  endif
...
static unsigned char * F_LOCAL	CheckClassExpression (unsigned char *, int,
						      bool);
static bool F_LOCAL	Initialise (char *);
...
static void F_LOCAL	ConvertUnixPathToMSDOS (void);
...
/* Process the options */
    ...
    while ((sc = GetOptions (argc, argv,
			     "MPRabc:defghijklmnopqrtsuvwxyz0",
			     GETOPT_MESSAGE)) != EOF)
    {
	switch (sc)
	{
	    ...
	    case 'M':
		GlobalFlags |= FLAGS_MSDOS_FORMAT;
	        break;
 ...
 */
	...
	if (((fid = S_open (FALSE, s->file = name, O_RMASK)) < 0) ||
	    ((s->u.file = ReOpenFile (ReMapIOHandler (fid), OpenReadMode))
	                == (FILE *)NULL))
	{
...
/* Read profile ? */
    ...
    if (((name != (char *)NULL) && (*name == CHAR_HYPHEN)) || level0)
	LoadTheProfileFiles ();
...
/* Check for exit */
	...
	{
	    if (wastty && (GlobalFlags & FLAGS_IGNOREEOF))
	    {
...
/* Assign the value */
    ...
    SecondAndRandomEV (String, atol (cp));
...
/* Restore the previous level information */
	...
	GlobalFlags = (unsigned int)(sp->GFlags & ~FLAGS_FUNCTION);
	flags = sp->Eflags;
...
/* Exclusive or inclusive class */
    ...
    if (not = C2bool ((*pattern == CHAR_MAGIC) &&
		      (*(++pattern) == CHAR_NOT)))
	pattern++;
...
static bool F_LOCAL Initialise (char *name)
{
    char	*s, *s1;
    char	**ap;
 ...
 *	      24 address.
 * For OS/2 : Set noignore interrupts and create the message queue for
 *	      session termination.
 */
 ...
 */
...
#if (OS_VERSION != OS_DOS) && (OS_VERSION != OS_DOS_32)
    name = PATH_TO_UNIX (StringSave (_APgmName));
#endif
    ...
    if ((s1 = strchr (s, CHAR_PERIOD)) != (char *)NULL)
	*s1 = 0;
...
#  if (OS_VERSION == OS_DOS) || (OS_VERSION == OS_DOS_32)
    SetVariableFromNumeric (LIT_OSmode, 0L);
#  else
    SetVariableFromNumeric (LIT_OSmode, 1L);
#  endif
#else
    SetVariableFromNumeric (LIT_OSmode, (long)_osmode);
#endif

#if (OS_VERSION == OS_OS2_32) || (OS_VERSION == OS_DOS_32)
...
/* Look for the next separator */
	...
	while ((cp = strchr (sp, CHAR_SEPARATOR)) != (char *)NULL)
	{
	    *cp = 0;

/* % in string ? */

	    if ((ap = strchr (ap, CHAR_FORMAT)) != (char *)NULL)
		*ap = 0;
 ...
 */
    ...
    if ((strchr (cp, CHAR_SEPARATOR) != (char *)NULL) ||
			...
			int  flag)		/* New status		*/
{
    ...
    if (IS_VariableFC (*vp->name))	/* not an internal symbol ($# etc) */
	vp->status |= flag;
}
...
#  endif
	...
	if ((rc = DosCreateQueue (&SessionQueueHandler, QUE_FIFO,
				  Name)) == NO_ERROR)
	    break;
...
#  ifdef FLAGS_EMACS
    if (strstr (ed, "emacs"))
    {
	GlobalFlags &= ~FLAGS_EDITORS;
	GlobalFlags |= FLAGS_EMACS;
    }
#  endif
